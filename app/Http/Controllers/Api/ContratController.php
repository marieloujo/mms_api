...
use App\Http\Resources\contrat\PortefeuillesResource;
...
class ContratController extends Controller
{
    ...
    public function save(Request $request){
		...
		try {
		    ...
		    if($request['client']['id'] == 0){
                ...
                $this->client=$this->clientService->create($client_request);
            ...
            if($request['assure']['id'] == 0){
                ...
                $this->assure=$this->assurerService->create($assure_request);
            ...
            foreach ($documents_data as $document){
                $i++;
               // $this->createImageFromBase64($document,$contrat->numero_contrat);
                ...
                $document=$this->documentService->create($document_data_request);
 ...
 /*                $benefice_beneficiaire_data=$benefice['beneficiaire']['user'];
                ...
                $this->beneficiaire=$this->beneficiaireService->create($benefice_beneficiaire_request);
    ...
    public function getContratData(array $request,$client_id,$assure_id){
        ...
        $contrat_code = $request['marchand']['user']['commune']['departement']['code'].
        $request['marchand']['user']['userable']['super_marchand_id'].
